...
def load_cities(path):
    ...
    with open(path, newline="", encoding="utf-8") as f:
        for r in csv.DictReader(f):
            cities.setdefault(r["station_id"], []).append(
...
def main():
    cities = load_cities("data/stations.csv")
    ...
    with open(
        ...
        for i in range(TARGET_SENTENCES):
            ...
            dep_entry = random.choice(cities[dep])
            arr_entry = random.choice(cities[arr])

            # Randomly choose between station_name and libelle_city
            dep_v = random.choice([dep_entry["station_name"], dep_entry["city"]])
            arr_v = random.choice([arr_entry["station_name"], arr_entry["city"]])
